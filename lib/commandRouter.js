...
class CommandRouterError extends Error {
...
const callWebhook = async ({ url, command, args }) => {
...
const triggerGitHubDispatch = async ({ command, args, token, repo }) => {
...
const callHttpEndpoint = async ({ command, args }) => {
  const url = args?.url;
...
export const dispatchAutomationCommand = async ({
  command,
  args = {},
  env = process.env,
} = {}) => {
  ...
  if (N8N_COMMANDS.has(base)) {
    ...
    return callWebhook({ url, command: normalized, args });
  }
  ...
  if (base === "http") {
    return callHttpEndpoint({ command: normalized, args });
  }
